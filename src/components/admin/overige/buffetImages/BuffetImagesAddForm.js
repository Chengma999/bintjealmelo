...
function BuffetImagesAddForm({ buffetImages, addBuffetImage }) {
  ...
  const handleSubmit = async (event) => {
    ...
    const index = buffetImages.findIndex((image) => image.name === name);
    if (index > -1) {
    ...
    addBuffetImage({
      name,
      src,
      sort_number: parseFloat(sort_number),
  ...
  return (
    <div style={{ margin: "auto auto", width: "80%" }}>
      <Divider orientation="left">Buffet Images</Divider>
...
export default BuffetImagesAddForm;
